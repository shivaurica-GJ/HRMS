...
export default function Attendance() {
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/attendance/monthly?month=${
            month + 1
          }&year=${year}`,
          {
  ...
  const filteredEmployees = employeesMap.filter(
    (emp) =>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <input
            ...
            className="pl-3 pr-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 w-full sm:w-auto"
          />
          <select
            value={month}
                  ...
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    {emp.data.map((val, i) => (
                      <td key={i} className="text-center px-1 py-4">
                        <div
                          className={`w-7 h-7 mx-auto flex items-center justify-center rounded-full text-xs font-bold ${
                            val === "P"
